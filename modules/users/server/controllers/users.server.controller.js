...
exports.signin = function (req, res, next) {
  passport.authenticate('local', function (err, user, info) {
    if (err || !user) return res.status(422).send({ message: 'ログインIDまたはパスワードが間違います。' });
    ...
    req.login(user, function (err) {
      if (err) {
        logger.error(err);
        return res.status(400).send({ message: 'ログインIDまたはパスワードが間違います。' });
      }
...
exports.password = function (req, res) {
  if (!req.user) return res.status(400).send({ message: 'ログインしていません。' });
  ...
  User.findById(req.user._id, function (err, user) {
    if (err || !user)
      return res.status(400).send({ message: 'アカウント情報が見つかりません。' });
