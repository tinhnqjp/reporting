...
function exportClean(report) {
  ...
  function write_basic(workbook, sheet, report) {
    ...
    if (report.signature && fs.existsSync('.' + report.signature)) {
      ...
      // });
      sheet.addImage(signature, getDimension('.' + report.signature, 42, 48, 84.1, 88.1, 24, 23));
  ...
  function write_other_works(sheet, report, sheetNo) {
    ...
    report.clean.other_works.forEach((other, index) => {
      if (row <= 61 && checkSheet(sheetNo, limit, index)) {
        ...
        row++;
        sheet.getCell('AH' + row).value = '　　' + other.detail;
        row++;
      }
    });
  }
...
function exportRepair(report) {
  ...
  function write_basic(workbook, sheet, report) {
    ...
    if (report.signature && fs.existsSync('.' + report.signature)) {
      var signature = addImage(workbook, report.signature);
      sheet.addImage(signature, getDimension('.' + report.signature, 23, 33, 61.1, 67.1, 20, 20));
    }
  }
  function write_works(sheet, report, sheetNo) {
    var limit = 8;
...
function exportConstruct(report) {
  ...
  function write_basic(workbook, sheet, report) {
    ...
    sheet.getCell('R6').value = report.manager;
    sheet.getCell('R9').value = report.saler;
    sheet.getCell('D8').value = report.construct.work_kind;
    var work_result = '';
    if (report.work_result === true) {
      work_result = '完了';
    }
    ...
    if (report.signature && fs.existsSync('.' + report.signature)) {
      var signature = addImage(workbook, report.signature);
      sheet.addImage(signature, getDimension('.' + report.signature, 24, 34, 56.1, 62.1, 39, 29));
    }
  }
  function write_works(sheet, report, sheetNo) {
    var limit = 8;
...
function exportPicture(report) {
  return new Promise((resolve, reject) => {
    ...
    workbook.xlsx.readFile(TEMPLATE_PATH)
      .then(function () {
        ...
        wsExport.pageSetup.printArea = 'A1:AA49';
        ...
        for (var index = 1; index <= total; index++) {
          ...
          copySheet.pageSetup.printArea = 'A1:AH78';
          var ws = _.cloneDeep(wsTemplate);
          copySheet.model = Object.assign(ws.model, {
            mergeCells: ws.model.merges
          });
  ...
  function write_basic(workbook, sheet, report) {
    sheet.getCell('J15').value = report.supplier;

    if (report.start) {
      var startStr = moment(report.start).format('YYYY年MM月DD日(ddd)/HH:mm');
      var starts = startStr.split('/');
      sheet.getCell('J17').value = starts[0];
      sheet.getCell('J19').value = starts[1];
    }

    sheet.getCell('G46').value = report.saler;
    sheet.getCell('R46').value = report.manager;
    if (report.location) {
      var locations = report.location.replace('　', '\r\n');
      sheet.getCell('C44').value = locations;
    }
    ...
    if (report.picture.store_image && fs.existsSync('.' + report.picture.store_image)) {
      ...
      sheet.addImage(store_image, getDimension('.' + report.picture.store_image, 4, 23, 22, 35, 23, 21));
    }
  }
  ...
  function write_set(workbook, sheet, set, row) {
    if (set.before && fs.existsSync('.' + set.before)) {
      // var dimensionb = getDimension('.' + set.before, 0, 17, (row), (row + 13), 17, 14);
      var dimensionb = getDimension('.' + set.before, 1, 16, (row + 1), (row + 14), 17, 14);
      var before = addImage(workbook, set.before);
      sheet.addImage(before, dimensionb);
    }
    if (set.after && fs.existsSync('.' + set.after)) {
      // var dimensiona = getDimension('.' + set.after, 17, 34, (row), (row + 13), 17, 14);
      var dimensiona = getDimension('.' + set.after, 17, 34, (row + 1), (row + 14), 17, 14);
      var after = addImage(workbook, set.after);
      sheet.addImage(after, dimensiona);
    }
    sheet.getCell('D' + (row + 15)).value = set.comment;
  }
}
